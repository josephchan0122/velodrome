...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        queue = sqs.get_queue_by_name(QueueName='-'.join((
            settings.SMART_BICYCLE_SQS_QUEUE_NAME,
        ...
        while True:
            try:
                for message in queue.receive_messages(
                    ...
                    update_bicycle_metadata(json.loads(message.body))
                    message.delete()
            except Exception as e:
                logger.exception('Unexpected exception when draining smart'
                                 ' bicycle updates messages: %s', e)
